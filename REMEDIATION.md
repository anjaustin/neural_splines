...
|---|---|---|---|---|---|
| C0 | Decide: repair, mark experimental, or cut | — | High | `needs decision` | **The bugs are fixed and the evidence got stronger, not weaker.** The demo now runs end to end and reports a relative output difference of **0.978** — its own "significant differences" verdict. Optimally fitted control points reconstruct a trained weight matrix at ~99% relative error, where 1.0 means no better than predicting zeros. Nothing in C1-C5 changes that; the limitation is the representation, not the code. |
